...
class FlaxCogVideoXCausalConv3d(nnx.Module):
    """
    A 3D causal convolution layer with conv_cache support for CogVideoX.
    
    This is the JAX/Flax implementation of CogVideoXCausalConv3d that matches
    the PyTorch version's functionality including:
    - Causal padding in the time dimension
    - Conv cache mechanism for efficient long sequence processing
    - Support for both 'constant' and 'replicate' padding modes
    
    Args:
        in_channels (int): Number of input channels
        out_channels (int): Number of output channels
        kernel_size (int or Tuple[int, int, int]): Kernel size
        stride (int): Stride (applied only to time dimension)
        dilation (int): Dilation rate
        pad_mode (str): Padding mode, either 'constant' or 'replicate'
    ...
    def __call__(self, inputs: jnp.ndarray, conv_cache: Optional[jnp.ndarray] = None):
        """
        Forward pass with causal padding and conv_cache support.
        
        Args:
            inputs: Input tensor of shape (B, T, H, W, C)
            conv_cache: Previous frames cache of shape (B, time_kernel_size-1, H, W, C)
            
        Returns:
            output: Convolution output
            new_cache: Updated cache for next iteration
        """
...
class FlaxCogVideoXSpatialNorm3D(nnx.Module):
    ...
    def __call__(
        ...
        Args:
            ...
            conv_cache: Cache dictionary for causal convolutions
            
        Returns:
            Normalized and conditioned features
            Updated conv_cache dictionary
        """
        new_conv_cache = {}
...
class FlaxAutoencoderKLCogVideoX(nnx.Module):
    ...
    def _decode(self, z: jnp.ndarray, zq: jnp.ndarray, deterministic: bool = True) -> jnp.ndarray:
        """
        Internal decode with frame batching.
