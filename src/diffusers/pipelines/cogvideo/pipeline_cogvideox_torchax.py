...
"""
CogVideoX Pipeline with TorchAx/TPU support.

This version adds:
- Data parallel (use_dp) support for CFG
- JAX/TPU sharding utilities
- TorchAx VAE decode integration
"""
...
def apply_input_sharding(tensor, env, use_dp=False):
    ...
    # For latents and similar 5D tensors (batch, frames, channels, height, width)
    # CogVideoX uses [B, T, C, H, W] format
    if ndim == 5:
...
class CogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    Pipeline for text-to-video generation using CogVideoX with TorchAx/TPU support.
    ...
    def __call__(
        ...
        Args:
            ...
            use_dp (`bool`, defaults to `False`):
                TPU/JAX specific: Whether to use data parallelism by stacking cond/uncond batches.
        ...
        if use_dp:
            assert do_classifier_free_guidance, "use_dp requires classifier free guidance"
        # =============================================================================
        ...
        # For non-DP mode, concat embeddings (original behavior)
        if not use_dp and do_classifier_free_guidance:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if use_dp:
                    # Stack cond and uncond for parallel processing
